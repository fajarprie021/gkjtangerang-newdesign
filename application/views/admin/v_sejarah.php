  ...
  <div class="mx-auto max-w-7xl">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-12 gap-4">
        <div>
            ...
            ?>
            <p class="text-gray-600 font-serif mt-2">Kelola konten sejarah gereja.</p>
        </div>
                ...
                <tbody class="text-gray-700 font-serif text-sm">
                    <?php
                        ...
                        foreach ($datatentang as $i) :
                          ...
                          $kategori_nama=$i->tulisan_kategori_nama; // Note: adjusted from original inconsistent naming if needed, but keeping original variable names
                    ?>
                    ...
                    <?php if($no == 0): ?>
                    <tr>
                        <td colspan="6" class="p-12 text-center text-gray-400 italic">Belum ada konten sejarah.</td>
                    </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<div id="ModalAdd" class="hidden fixed inset-0 z-[99] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-opacity">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <form action="<?php echo base_url().'admin/sejarah/simpan_sejarah'?>" method="post" enctype="multipart/form-data" class="flex flex-col h-full">
            <div class="bg-primary px-6 py-4 flex justify-between items-center text-white shrink-0">
                <h4 class="font-headline tracking-widest text-lg">Tambah Sejarah</h4>
            ...
            <div class="p-8 font-serif overflow-y-auto space-y-6">
                <div>
                    <label class="block text-primary font-headline text-sm mb-2 capitalize">Judul Sejarah</label>
                    <input type="text" name="xjudul" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:border-secondary outline-none transition-all" placeholder="Misal: Sejarah GKJ Tangerang" required>
                </div>
                <div>
                ...
                <div>
                    <label class="block text-primary font-headline text-sm mb-2 capitalize">Konten Berita</label>
            ...
            <div class="bg-gray-50 px-8 py-5 flex justify-end gap-3 border-t border-gray-100 shrink-0">
                ...
                <button type="submit" class="px-8 py-3 bg-secondary text-white rounded-md hover:bg-primary font-headline text-sm uppercase tracking-wider shadow-sm transition-colors uppercase">Simpan Sejarah</button>
            </div>
        </form>
    </div>
</div>
...
<div id="ModalEdit<?php echo $tulisan_id;?>" class="hidden fixed inset-0 z-[99] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-opacity">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[95vh]">
        <form action="<?php echo base_url().'admin/sejarah/update_sejarah'?>" method="post" enctype="multipart/form-data" class="flex flex-col min-h-0 h-full">
            <div class="bg-primary px-6 py-4 flex justify-between items-center text-white shrink-0 shadow-sm">
                <h4 class="font-headline tracking-widest text-lg">Update Sejarah</h4>
            ...
            <div class="p-8 font-serif overflow-y-auto grow space-y-6 scrollbar-thin scrollbar-thumb-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-6">
                        <div>
                            <label class="block text-primary font-headline text-[10px] uppercase tracking-[0.2em] font-bold mb-2">Judul Sejarah</label>
                ...
                <div>
                    <label class="block text-primary font-headline text-[10px] uppercase tracking-[0.2em] font-bold mb-2 text-center">Isi Sejarah</label>
            ...
            <div class="bg-gray-50 px-8 py-5 flex justify-end gap-3 border-t border-gray-100 shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                ...
                <button type="submit" class="px-8 py-3 bg-secondary text-white rounded-md hover:bg-primary font-headline text-[10px] uppercase tracking-[0.2em] shadow-lg transition-all hover:scale-[1.02] active:scale-95 font-bold">Update Sejarah Sekarang</button>
            </div>
        </form>
    </div>
</div>
...
<div id="ModalHapus<?php echo $tulisan_id;?>" class="hidden fixed inset-0 z-[99] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-opacity">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden p-8 text-center font-serif">
        <form action="<?php echo base_url().'admin/sejarah/hapus_sejarah'?>" method="post">
            <div class="w-16 h-16 bg-red-100 text-red-600 rounded-full flex items-center justify-center mx-auto mb-4">
                <span class="material-symbols-outlined text-3xl">warning</span>
            </div>
            <h4 class="font-headline text-xl text-primary mb-2">Hapus Konten?</h4>
            <p class="text-gray-500 text-sm mb-8 px-4">Anda yakin ingin menghapus data sejarah <strong><?php echo $tulisan_judul;?></strong>? Tindakan ini tidak dapat dibatalkan.</p>
