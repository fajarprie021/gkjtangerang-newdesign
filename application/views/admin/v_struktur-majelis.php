    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-12 gap-4">
        <div>
            ...
            $this->load->view('components/section_header_left', array(
                ...
                'title'   => 'Struktur Majelis'
            )); 
            ?>
            <p class="text-gray-600 font-serif mt-2">Kelola konten struktur majelis gereja.</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse min-w-[900px]">
                ...
                <tbody class="text-gray-700 font-serif text-sm">
                    ...
                    <?php if($no == 0): ?>
                    <tr>
                        <td colspan="6" class="p-12 text-center text-gray-400 italic">Belum ada konten struktur majelis.</td>
                    </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<div id="ModalAdd" class="hidden fixed inset-0 z-[99] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-opacity">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <form action="<?php echo base_url().'admin/strukturmajelis/simpan_strukturmajelis'?>" method="post" enctype="multipart/form-data" class="flex flex-col h-full">
            <div class="bg-primary px-6 py-4 flex justify-between items-center text-white shrink-0">
                <h4 class="font-headline tracking-widest text-lg">Tambah Struktur Majelis</h4>
            ...
            <div class="p-8 font-serif overflow-y-auto space-y-6">
                <div>
                    ...
                    <input type="text" name="xjudul" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:border-secondary outline-none transition-all" placeholder="Misal: Struktur Majelis GKJ Tangerang" required>
                </div>
                <div>
            ...
            <div class="bg-gray-50 px-8 py-5 flex justify-end gap-3 border-t border-gray-100 shrink-0">
                ...
                <button type="submit" class="px-8 py-3 bg-secondary text-white rounded-md hover:bg-primary font-headline text-sm uppercase tracking-wider shadow-sm transition-colors uppercase">Simpan Struktur</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[95vh]">
        <form action="<?php echo base_url().'admin/strukturmajelis/update_strukturmajelis'?>" method="post" enctype="multipart/form-data" class="flex flex-col min-h-0 h-full">
            ...
            <div class="p-8 font-serif overflow-y-auto grow space-y-6 scrollbar-thin scrollbar-thumb-gray-200">
                ...
                <div>
                    <label class="block text-primary font-headline text-[10px] uppercase tracking-[0.2em] font-bold mb-2 text-center">Isi Struktur Majelis</label>
            ...
            <div class="bg-gray-50 px-8 py-5 flex justify-end gap-3 border-t border-gray-100 shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                ...
                <button type="submit" class="px-8 py-3 bg-secondary text-white rounded-md hover:bg-primary font-headline text-[10px] uppercase tracking-[0.2em] shadow-lg transition-all hover:scale-[1.02] active:scale-95 font-bold">Update Struktur Sekarang</button>
            </div>
        </form>
    </div>
</div>
...
<div id="ModalHapus<?php echo $tulisan_id;?>" class="hidden fixed inset-0 z-[99] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-opacity">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden p-8 text-center font-serif">
        <form action="<?php echo base_url().'admin/strukturmajelis/hapus_strukturmajelis'?>" method="post">
            <div class="w-16 h-16 bg-red-100 text-red-600 rounded-full flex items-center justify-center mx-auto mb-4">
                <span class="material-symbols-outlined text-3xl">warning</span>
            </div>
            <h4 class="font-headline text-xl text-primary mb-2">Hapus Konten?</h4>
            <p class="text-gray-500 text-sm mb-8 px-4">Anda yakin ingin menghapus data struktur <strong><?php echo $tulisan_judul;?></strong>? Tindakan ini tidak dapat dibatalkan.</p>
