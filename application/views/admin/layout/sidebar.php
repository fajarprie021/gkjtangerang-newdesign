<aside id="admin-sidebar" class="fixed inset-y-0 left-0 z-40 w-64 bg-primary transition-transform duration-300 transform md:translate-x-0 -translate-x-full md:relative md:flex flex-col border-r border-secondary/20 shadow-xl">
            ...
            <?php if ($has_submenu): ?>
                ...
                <div x-data="{ open: <?php echo $is_active_parent ? 'true' : 'false'; ?> }" class="space-y-1">
                    <button @click="open = !open" 
                        ...
                        <span class="material-symbols-outlined transition-transform duration-200 text-[18px] flex-shrink-0 ml-2" :class="open ? 'rotate-180' : ''">expand_more</span>
                    </button>
                    ...
                    <div x-show="open" 
                        ...
                        <?php foreach($query->result() as $submenu): 
                            ...
                            $is_child_active = ($this->uri->segment(2) == $resultReplace_NamePage);
                        ?>
                            <?php if ($submenu->sub_menu_status == 1): ?>
                                <a href="<?php echo base_url('admin/').$resultReplace_NamePage;?>" 
                                   class="flex items-center gap-3 pl-14 pr-6 py-3 text-[10px] font-medium uppercase tracking-[0.2em] transition
...
<div id="sidebar-overlay" 
     class="fixed inset-0 z-30 bg-black/50 backdrop-blur-sm hidden md:hidden"
